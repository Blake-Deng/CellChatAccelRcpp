...
The archived v0.1.0 release is available at \url{https://doi.org/10.5281/zenodo.21186108}.
...
Single-cell RNA sequencing has made it possible to describe cellular states across tissues, diseases and perturbations at atlas scale \citep{regev2017human,hao2021integrated}.
...
CellChat provides a widely adopted R framework for inferring and interpreting communication networks from single-cell expression data \citep{armingol2021deciphering,dimitrov2022comparison,jin2021cellchat}.
...
Both engines were run under the same benchmark runner and hardware environment.
Grid tasks were scheduled as independent \texttt{Rscript} jobs; within each paired job, the baseline and accelerated engines were executed serially from the same prepared CellChat object.
All runs used one R process per benchmark job unless otherwise stated, with \texttt{OMP\_NUM\_THREADS}, \texttt{OPENBLAS\_NUM\_THREADS}, \texttt{MKL\_NUM\_THREADS} and \texttt{BLIS\_NUM\_THREADS} fixed to 1.
Concurrent scheduling of independent jobs was used only for batch throughput and not as an engine-specific optimization.
...
The archived v0.1.0 software release is available at \url{https://doi.org/10.5281/zenodo.21186108}.
